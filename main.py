...
def student():
     if 'user' in session:
          ...
          if(g.user):
               ...
               no=df.shape[0]
               m0=df['s_gender'].tolist().count('M')
               f0=df['s_gender'].tolist().count('F')
               ...
               alll=[]
               alll.append("All Branches")
               og1.append(alll)
               alll=[]
               alll.append("All Years")
               og.append(alll)
               d_drop.append("All Programs")
               ...
               mcc=m['Caste'].value_counts().sort_index().tolist()
               mcc2=m['Caste'].value_counts().sort_index().index.tolist()

               f=drop_data.loc[drop_data['s_gender']=="F"]
               fcc=f['Caste'].value_counts().sort_index().tolist()
               fcc2=f['Caste'].value_counts().sort_index().index.tolist()

               caa= list(set(mcc2) | set(fcc2))

               al = {caa[i-1]: i for i in range(1,len(caa)+1)}
               alpha2=[]
               for i in al:
                    if(i not in mcc2):
                         mcc2.insert(al[i]-1,i)
                         mcc.insert(al[i]-1,0)
               for i in al:
                    if(i not in fcc2):
                         fcc2.insert(al[i]-1,i)
                         fcc.insert(al[i]-1,0)
               ...
               glistt=[mcc,fcc]
               gtabb=pd.DataFrame(glistt,columns=alpha2,index=['M','F'])
               l=len(drop_data)
               ...
               return render_template("student.html",c=c,s_branch=s_branch,mcc=mcc,fcc=fcc,alpha2=alpha2,g_tables1=[gtabb.to_html(classes='data', header="true")],g_values=g_values,g_names=g_names,f_values=f_values,f_names=f_names,l=l,tables=[drop_data.to_html(classes='data', header="true",index_names=False, index=False)],s_year=s_year,s_dept=s_dept,b_drop=b_drop,yd=yd,og=og,dd=dd,og1=og1,mc=mc,fc=fc,alpha1=alpha1,g_tables=[gtab.to_html(classes='data', header="true")],year_drop=year_drop,d_drop=d_drop,pg_year_count=pg_year_count,pg_years=pg_years,u=u,p=p,ph=ph,le_count=le_count,le_branch=le_branch,course_names=course_names,course_values=course_values,alpha=alpha,ug_branch=ug_branch,ug_branch_count=ug_branch_count,ug_years=ug_years,ug_year_count=ug_year_count,pg_branch=pg_branch,pg_branch_count=pg_branch_count,phd_branch=phd_branch,phd_branch_count=phd_branch_count,year_count_names=year_count_names,no=no,m0=m0,f0=f0,name=session['user'])
     else:
          return redirect(url_for('home'))
...
def exams():
     if 'user' in session:
          g.user=session['user']
          if(g.user):
               return render_template("exams.html",name=session['user'])
          else:
               return render_template("mainpage.html")
     else:
          return redirect(url_for('home'))
...
def faculty():
     if 'user' in session:
          ...
          if(g.user):
               ...
               og.append(alll)
               # pr_drop.append("All Programs")

               yd= {pr_drop[i-1]: i for i in range(1,len(pr_drop)+1)}


               s_pr='all'
               s_dept = 'Information Technology'
               if request.method=='POST':
                    res_Det=request.form
               ...
               f_valuess=drop_data['f_course'].value_counts().tolist()
               f_namess=drop_data['f_course'].value_counts().index.tolist()

               g_valuess=drop_data['f_gender'].value_counts().tolist()
               g_namess=drop_data['f_gender'].value_counts().index.tolist()

               mm=drop_data.loc[drop_data['f_gender']=="M"]
               mcc=mm['Caste'].value_counts().sort_index().tolist()
               mcc2=mm['Caste'].value_counts().sort_index().index.tolist()

               ff=drop_data.loc[drop_data['f_gender']=="F"]
               fcc=ff['Caste'].value_counts().sort_index().tolist()
               fcc2=ff['Caste'].value_counts().sort_index().index.tolist()

               caa= list(set(mcc2) | set(fcc2))

               all = {caa[i-1]: i for i in range(1,len(caa)+1)}
               alphaa=[]

               for i in all:
                    if(i not in mcc2):
                         mcc2.insert(all[i]-1,i)
                         mcc.insert(all[i]-1,0)
               for i in all:
                    if(i not in fcc2):
                         fcc2.insert(all[i]-1,i)
                         fcc.insert(all[i]-1,0)
               ...
               gtabb=pd.DataFrame(glistt,columns=alphaa,index=['M','F'])
               l=len(drop_data)
...
def programs():
     if 'user' in session:
          ...
          if(g.user):
               ...
               d1=len(df['Department'].value_counts())

               pr_count1=df['Program'].value_counts().to_list()
               pr1=df['Program'].value_counts().index.to_list()
               ...
               if request.method=='POST':
                    dropDetails=request.form
                    s_year=dropDetails['selected_year']
               ...
               return render_template("programs.html",pr_count1=pr_count1,pr1=pr1,d1=d1,d_count=d_count,s=s,s_count=s_count,l=l,tables=[drop_data.to_html(classes='data', header="true",index_names=False, index=False)],s_year=s_year,intake=intake,yr_count=yr_count,yr=yr,pr_count=pr_count,pr=pr,p=p,d=d,year_drop=year_drop,school=school,name=session['user'])
     else:
          return redirect(url_for('home')) 
...
def admissions():
     if 'user' in session:
          ...
          if(g.user):
               ...
               ad_data=drop_data.groupby('Program')['Total Admitted'].sum()
               ad_data=pd.DataFrame(ad_data)
               d_names=ad_data.index.to_list()
               ad_amount=ad_data['Total Admitted'].to_list()


               pr_data=drop_data.groupby('Program')['Lateral Count'].sum()
               pr_data=pd.DataFrame(pr_data)
               pr=pr_data.index.to_list()
               le=pr_data['Lateral Count'].to_list()

               l=len(drop_data)
               drop_data.rename(columns = {'Admitted percent':'Admitted %','Lateral Count':'LE Admitted'}, inplace = True)
               sum_list=['Intake','Total Admitted']

               # drop_data.loc['Total'] = drop_data[sum_list].sum().astype(int)
               drop_data.loc['Column_Total']= drop_data.sum(numeric_only=True, axis=0)

               # for i in drop_data:
               #      if(drop_data[i].dtype=='float' and i!='Admitted %'):
               #           drop_data[i] = drop_data[i].astype(int)

               drop_data['Intake'] = drop_data['Intake'].astype(int)
               drop_data['Total Admitted'] = drop_data['Total Admitted'].astype(int)
               drop_data['Admitted'] = drop_data['Admitted'].astype(int)
               drop_data['LE Admitted'] = drop_data['LE Admitted'].astype(int)
               drop_data['S.No'] = drop_data['S.No'].astype(int)

               # for i in drop_data:
               #      if(drop_data[i].dtype)=='float':
               #           drop_data[i] = drop_data[i].astype(int)


               drop_data.iloc[-1:,0]='Total'
               clear=[1,2,4,5]

               drop_data.fillna('', inplace=True)
               for i in clear:
                  drop_data.iloc[-1:,i]=''

               drop_data.iloc[-1:,-1:]=drop_data.iloc[-1:,6]/drop_data.iloc[-1:,3]*100
               drop_data['Admitted %'] = drop_data['Admitted %'].round(decimals = 2)

               return render_template("admissions.html",le=le,ad_amount=ad_amount,d_names=d_names,intake_amount=intake_amount,dept_names=dept_names,pr=pr,l=l,tables=[drop_data.to_html(classes='data', header="true",index_names=False, index=False)],s_year=s_year,lr=lr,lr_count=lr_count,yr_count1=yr_count1,yr1=yr1,yr_count=yr_count,yr=yr,p=p,d=d,year_drop=year_drop,name=session['user'])
     else:
          return redirect(url_for('home')) 
...
def a_drop():
     ...
     drop_data.iloc[-1:,-1:]=drop_data.iloc[-1:,6]/drop_data.iloc[-1:,3]*100
     # drop_data.iloc[-1:,-1:].round()


     # drop_data['Admitted %'] = drop_data

     # drop_data = pd.concat([drop_data,pd.DataFrame(drop_data.sum(numeric_only=True,axis=0),columns=['Grand Total']).T])
     ...
     if 'user' in session:
          ...
          if(g.user):
               ...
               if request.method=='POST':
                    dropDetails=request.form
                    s_year=dropDetails['selected_year']
                    s_dept=dropDetails['selected_dept']
               ...
               return render_template("research.html",inv=inv,amt=amt,l=l,tables=[drop.to_html(classes='data', header="true",index_names=False, index=False)],drop_data=drop_data,drop=drop,s_year=s_year,s_dept=s_dept,yd=yd,og=og,year_drop=year_drop,dept=dept,agency=agency,sanc_amount_agency=sanc_amount_agency,year=year,sanc_amount_year=sanc_amount_year,dept_names=dept_names,sanc_amount=sanc_amount,status=status,status_count=status_count,name=session['user'])
     else:
          return redirect(url_for('home'))             
